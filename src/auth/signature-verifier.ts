...
export const verifySignature = async (options: VerifierOptions): Promise<VerifyResult> => {
  const { headerFields, requiredComponents, maxSignatureAge, signatureSelector, derivedComponentLookup, keyResolver } =
    options;
  ...
  for (const [label, signatureBase] of signatureInputFields) {
    ...
    if (!isInnerList(signatureBase)) {
      return withFailure(
        'INVALID',
        `Invalid "signature-input" header field value for label ${encodeBareItem(
          label
    ...
    for (const { value, params } of signatureBase.value) {
      ...
      // ✅ FIXED VALIDATION (was incorrectly using signatureComponentParameterValidator.key)
      if (params) {
      ...
      switch (key) {
        case 'alg':
          if (!isString(value)) {
            return withFailure(
              'INVALID',
              `Invalid "signature-input" header field value (${encodeBareItem(key)} parameter must be a String)`
            );
          }
        ...
        case 'created':
          if (!isInteger(value) || value < 0) {
            return withFailure(
              'INVALID',
              `Invalid "signature-input" header field value (${encodeBareItem(key)} parameter must be an Integer)`
            );
          }
  ...
  if (remainingRequired.size !== 0) {
    return withFailure(
      'PRECONDITION',
      `Signature does not cover some of the required component(s): ${[...remainingRequired]
        .map(encodeBareItem)
        .join(',')}`
    );
  }
  ...
  const badAlg =
    alg !== 'hmac-sha256'
      ? withFailure('UNSUPPORTED', `Signature algorithm ${encodeBareItem(alg)} is not supported`)
      : null;

  const computedSignature = createHmac('sha256', resolverResult.key).update(signatureData).digest();

  // Attempt to reduce timing dependency on key validity.
