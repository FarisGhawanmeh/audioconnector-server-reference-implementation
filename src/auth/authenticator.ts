...
async function verifyRequestSignatureImpl(
  ...
  const result = await verifySignature({
    ...
    derivedComponentLookup: (name) => {
      ...
      if (name === '@authority') {
        // Genesys signs @authority; in Node/Express this maps to the Host header.
    ...
    keyResolver: async (parameters) => {
      ...
      // IMPORTANT: empty Uint8Array should be treated as missing key
      if (!secret || secret.length === 0) {
        return withFailure('BADKEY', `No secret found for keyid="${keyId}"`);
      }
